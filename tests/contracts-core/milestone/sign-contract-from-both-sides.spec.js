...
const { personalDetails } = milestoneContractData;

test.describe('Client and Contractor - Milestone contract signing', () => {
  ...
  test(
    'Client and Contractor should sign Milestone contract successfully @contract-core-qa-front',
    {
      tag: ['@slowRegression'],
    },
    async ({ page, request }) => {
      ...
      // Assert Special clause
      await expect(page.locator(contractDetailsTab.specialClause)).toHaveText(contract.workStatements[0].specialClause);
