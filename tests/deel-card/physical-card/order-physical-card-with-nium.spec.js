...
import {
  agreeTermsOfService,
  agreeUsingBillingAddress,
  assertPhysicalDeelCardCreation,
  fillPhoneNumber,
  fillReferenceNumber,
  uploadPoaDoc,
...
import { selectOptionFromDropdownBySearchText } from '../../../helpers/shared-functions/select-options';
...
test.describe('Order a physical deel card using nium', () => {
  let client;
  let contractor;
  let testData;
  ...
  test.beforeEach('Create users, contracts, and sign them via API', async ({ request }) => {
    ...
    client = await createClientWithNonUSEntity({ country: country.US.value });
  ...
  test(
    'Order a physical card successfully via nium card issuer @payment-cards-qa-front',
    {
      tag: ['@slowRegression'],
    },
    async ({ page, request }) => {
      ...
      // Click and clears the onboarding modal
      await clearsOnboardingModal(page);

      // Clicks and selects virtual card
      await selectPhysicalDeelCard(page);
      ...
      // Accepts and clicks the user of billing address
      await agreeUsingBillingAddress(page);

      // Selects the dialCode from the dropdown list of dial codes
      await selectOptionFromDropdownBySearchText(page, icDeelCardShippingAddress.dialCodeDropdown, testData.phoneCodeText);

      // Fills in the phone number input field
      await fillPhoneNumber(page, testData.phoneNumber);

      // Clicks the next button to proceed to the next step of Accepting ToS
      await nextStep(page);
      ...
      await assertPhysicalDeelCardCreation(page);
    }
  );
});
